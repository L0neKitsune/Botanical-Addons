...
class SlabColoredWood(val full: Boolean, val source: Block) : ShadowFoxSlabs(full, source.material)
{
    private val TYPES = 16

    init {
        setStepSound(source.stepSound)
        setBlockName(source.unlocalizedName.replace("tile.".toRegex(), "") + "Slab" + (if (full) "_Full" else ""))
    }
