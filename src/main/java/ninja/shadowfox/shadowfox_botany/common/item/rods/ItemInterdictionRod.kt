...
open class ItemInterdictionRod(name: String = "interdictionRod") : ItemMod(name), IManaUsingItem, IAvatarWieldable {
    ...
    override fun onUsingTick(stack: ItemStack?, player: EntityPlayer?, count: Int) {
        if (player != null) {
            ...
            var priest =  false //(ItemPriestEmblem.getEmblem(2, player) != null)
            ...
            if (ManaItemHandler.requestManaExactForTool(stack, player, cost, false)) {
                ...
                val entities = world.getEntitiesWithinAABB(Entity::class.java, AxisAlignedBB.fromBounds(x - range, y - range, z - range, x + range, y + range, z + range), { e -> e == player || e is EntityLivingBase && e !is EntityPlayer && e !is EntityDoppleganger })
    ...
    override fun onAvatarUpdate(tile: IAvatarTile, stack: ItemStack) {
        ...
        var world = te.world
        var x = te.pos.x.toDouble()
        var y = te.pos.y.toDouble()
        var z = te.pos.z.toDouble()

        if (tile.currentMana >= AVATAR_COST) {
            if (tile.elapsedFunctionalTicks % 5 == 0) particleRing(world, x, y, z, RANGE, 0f, 0f, 1f)

            val entities = world.getEntitiesWithinAABB(Entity::class.java,
                    AxisAlignedBB.fromBounds(x - RANGE, y - RANGE, z - RANGE,
                            x + RANGE, y + RANGE, z + RANGE), { e -> e is EntityLivingBase && e !is EntityPlayer && e !is EntityDoppleganger })
